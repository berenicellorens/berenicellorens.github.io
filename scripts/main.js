...
// This function returns a random integer given a offset and a range
rand = (offset, range) => Math.floor((Math.random() * range) + offset);
// This function fires up a `func`tion after a `time` in miliseconds
setRandTimeout = async (func, time) => setTimeout(func, rand(0, time));
